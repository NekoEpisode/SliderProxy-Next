...
public class ArgumentTypeData {
    ...
    public static ArgumentTypeData fromBrigadierType(ArgumentType<?> type) {
        ...
        if (type instanceof BoolArgumentType) {
            data.parserId = BRIGADIER_BOOL;
        } else if (type instanceof FloatArgumentType) {
            FloatArgumentType floatType = (FloatArgumentType) type;
            ...
            DoubleArgumentType doubleType = (DoubleArgumentType) type;
            ...
            LongArgumentType longType = (LongArgumentType) type;
            ...
            StringArgumentType stringType = (StringArgumentType) type;
            data.parserId = BRIGADIER_STRING;
            switch (stringType.getType()) {
                case SINGLE_WORD: data.stringBehavior = 0; break;
                case QUOTABLE_PHRASE: data.stringBehavior = 1; break;
                case GREEDY_PHRASE: data.stringBehavior = 2; break;
            }
        } else {
            // 默认字符串类型
            data.parserId = BRIGADIER_STRING;
    ...
    public void read(ByteBuf buf) {
        parserId = MinecraftProtocolHelper.readVarInt(buf);
        ...
        switch (parserId) {
            ...
            case BRIGADIER_STRING:
                stringBehavior = MinecraftProtocolHelper.readVarInt(buf);
                break;
            ...
            case MINECRAFT_RESOURCE_SELECTOR:
                registry = MinecraftProtocolHelper.readString(buf);
                break;
    ...
    public void write(ByteBuf buf) {
        MinecraftProtocolHelper.writeVarInt(buf, parserId);
        ...
        switch (parserId) {
            ...
            case BRIGADIER_STRING:
                MinecraftProtocolHelper.writeVarInt(buf, stringBehavior);
                break;
            ...
            case MINECRAFT_RESOURCE_KEY:
            case MINECRAFT_RESOURCE_SELECTOR:
                MinecraftProtocolHelper.writeString(buf, registry != null ? registry : "");
                break;
